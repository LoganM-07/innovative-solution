...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //move left unless on side or blockage
    function moveLeft() {
        undraw()
    ...
    //move right unless on side or blockage
    function moveRight() {
        undraw()
    ...
    function rotate() {
        undraw()
        currentRotation ++
        if(currentRotation === current.length) { //if the current rotation goes to 4, go back to 0
            currentRotation = 0
        }
        current = theTetronimoes[random][currentRotation]
        draw()
    ...
    //show up-next tetronimo in mini-grid
    const displaySquares = document.querySelectorAll('.mini-grid div')
    const displayWidth = 4
    const displayIndex = 0
    
    //the Tetromino display format
    ...
    //add functionality of pause buton
